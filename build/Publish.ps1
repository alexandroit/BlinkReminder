...
try {
    Invoke-Checked dotnet @('publish', 'src/BlinkReminder.App/BlinkReminder.App.csproj', '-c', 'Release', '-r', 'win-x64', '--self-contained', 'true', '-p:WindowsAppSDKSelfContained=true', '-p:WindowsPackageType=None', '-p:RestoreLockedMode=true', '-p:PublishSingleFile=false', '-p:PublishTrimmed=false', '-o', $destination)
